...
class Database:
    ...
    def getGeneralFriends(self, uid, pos, count):
        # For now, We just return a list of random contacts
        # TODO: Sort contacts based on 'Online' and then place the 'recent' contacts on the bottom of the list
        d = self.db
        try:
            b = d['users'].find_one({"_id":uid})
            m = list(b['connections']['friends'])
            o = len(m)
            if pos >= o :
                return None
            ll = o - pos
            ...
            g = list()
            for i in mm:
                h = self.getUserBasicInfo(i)
                h['stats-unseen'] = 0#i['unseen']
                g.append(h)
            return g
